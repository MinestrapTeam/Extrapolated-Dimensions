...
public class PotionList implements IPotionList
{
	...
	public void initPotionTypes()
	{
		...
		moveSpeed = new PotionType(new PotionEffect(Potion.moveSpeed.id, 20 * 180, 0), 7, 20 * 360, moveSlowdown, ECStacks.sugar, dashing);
		weakness = new PotionType(new PotionEffect(Potion.weakness.id, 20 * 90, 0), 2, 20 * 240, ECStacks.fermented_spider_eye, awkward);
		damageBoost = new PotionType(new PotionEffect(Potion.damageBoost.id, 20 * 180, 0), 4, 20 * 300, weakness, ECStacks.blaze_powder, awkward);
		harm = new PotionType(new PotionEffect(Potion.harm.id, 1, 0), 1, 0);
		heal = new PotionType(new PotionEffect(Potion.heal.id, 1, 0), 1, 0, harm, ECStacks.speckled_melon, thick);
		regeneration = new PotionType(new PotionEffect(Potion.regeneration.id, 20 * 45, 0), 2, 20 * 180, moveSlowdown, ECStacks.ghast_tear, awkward);
		fireResistance = new PotionType(new PotionEffect(Potion.fireResistance.id, 20 * 180, 0), 0, 20 * 360, moveSlowdown, ECStacks.magma_cream, awkward);
		invisibility = new PotionType(new PotionEffect(Potion.invisibility.id, 20 * 180, 0), 0, 720 * 20);
		nightVision = new PotionType(new PotionEffect(Potion.nightVision.id, 20 * 180, 0), 0, 20 * 300, invisibility, ECStacks.golden_carrot, thin);
		poison = new PotionType(new PotionEffect(Potion.poison.id, 20 * 45, 0), 2, 20 * 60, ECStacks.spider_eye, acrid);
	}
