...
public class TeleporterVirtious extends CustomTeleporter
{
	public TeleporterVirtious(WorldServer worldServer)
	{
		super(worldServer);
	}
	...
	public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float yaw)
	{
		...
		{
			PortalPosition VirtiousPortalPosition = (PortalPosition) this.destinationCoordinateCache
				                                                         .getValueByKey(chunkIntPair);
			d3 = 0.0D;
		...
		if (d3 >= 0.0D)
		{
			if (flag)
			{
				this.destinationCoordinateCache
					.add(chunkIntPair, new PortalPosition(i, j, k, this.world.getTotalWorldTime()));
