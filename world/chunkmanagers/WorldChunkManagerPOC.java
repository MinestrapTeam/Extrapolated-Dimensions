...
public class WorldChunkManagerPOC extends WorldChunkManager
{
	...
	public WorldChunkManagerPOC(World world)
	{
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] biomes, int x, int z, int width, int length)
	{
		...
		for (int i = 0; i < width * length; ++i)
		{
			...
			{
				biomes[i] = MDMWorld.biomePOCPlains;
			}
		}
	 ...
	 */
	@Override
	public ChunkPosition findBiomePosition(int x, int z, int off, List list, Random random)
	{
		IntCache.resetIntCache();
		int l = x - off >> 2;
		int i1 = z - off >> 2;
